...
export function EstimatorLinks({ stacked = false, compact = false }: EstimatorLinksProps) {
  return (
    <motion.div 
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4 }}
      className={`bg-gradient-to-br from-primary/5 via-background to-secondary/5 rounded-2xl border border-primary/10 shadow-lg ${compact ? 'p-3 sm:p-4' : stacked ? 'p-4 sm:p-6' : 'p-6 sm:p-8'}`}
    >
      ...
      {/* Cards */}
      <div className={compact ? "grid grid-cols-1 gap-3" : stacked ? "grid grid-cols-1 gap-6" : "grid grid-cols-1 md:grid-cols-2 gap-6"}>
          ...
          return (
            ...
            >
              <Card className={`overflow-hidden hover:shadow-xl transition-all duration-300 border-2 border-transparent hover:border-primary/20 bg-card h-full flex flex-col group hover:-translate-y-1`}>
                {/* Image Container - hide in compact mode */}
                {!compact && (
                ...
                {/* Content */}
                <div className={compact ? "p-3 flex flex-col flex-1" : "p-5 flex flex-col flex-1"}>
                  {/* Title Row */}
                  <div className={`flex items-center gap-3 ${compact ? 'mb-2' : 'mb-3'}`}>
                    <div className={`${compact ? 'w-8 h-8' : 'w-10 h-10'} rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0`}>
                      <Icon className={`${compact ? 'w-4 h-4' : 'w-5 h-5'} text-primary`} />
                    </div>
                    <div>
                      <h4 className={`font-bold text-foreground leading-tight ${compact ? 'text-base' : 'text-lg'}`}>{estimator.title}</h4>
                      {!compact && <p className="text-sm text-muted-foreground">{estimator.subtitle}</p>}
                    </div>
                  </div>
                  ...
                  {/* CTA Button */}
                  <Button asChild size={compact ? "sm" : "default"} className="w-full touch-target bg-primary hover:bg-primary/90">
